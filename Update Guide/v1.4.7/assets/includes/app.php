...
if ($music->loggedin && !empty($music->user) && !empty($music->pro_packages[$music->user->pro_type]) && $music->pro_packages[$music->user->pro_type]['ex_time'] != 0) {
    $end_time = $music->user->pro_time + $music->pro_packages[$music->user->pro_type]['ex_time'];
    if ($end_time <= time()) {
        $db->where('id',$music->user->id)->update(T_USERS,array('is_pro' => 0,
                                                                'pro_type' => 0,
                                                                'pro_time' => 0));
    }
}
