...
function userData($user_id = 0, $options = array()) {
    ...
    if ($fetched_data->verified == 1 && $music->config->verification_badge == 'on') {
        $fetched_data->name_v = $fetched_data->name . ' <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="#3d8cfa" stroke="#fff" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="feather feather-check-circle"><path d="M22 11.08V12a10 10 0 1 1-5.93-9.14"></path><polyline points="22 4 12 14.01 9 11.01"></polyline></svg>';
    }
...
function PT_UploadToS3($filename, $config = array()) {
    ...
    if ($music->config->ftp_upload == "on" && !empty($music->config->ftp_host) && !empty($music->config->ftp_username)) {
        ...
        $s3->setRegion($music->config->space_region);
        $uploaded = $s3->putObjectFile($filename,$music->config->space_name,$filename,'public-read');
        if ($uploaded == true) {
            if (empty($config['delete']) && empty($config['amazon'])) {
                @unlink($filename);
            }
            return true;
        }
        ...
        $s3->setRegion($music->config->region);

        $uploaded = $s3->putObjectFile($filename,$music->config->s3_bucket_name,$filename,'public-read');
        if ($uploaded == true) {
            if (empty($config['delete']) && empty($config['amazon'])) {
                $ob = $s3->getObjectInfo($music->config->s3_bucket_name, $filename);
                if (!empty($ob) && is_array($ob)) {
                    @unlink($filename);
                }
            }
            return true;
...
function PT_DeleteFromToS3($filename, $config = array()) {
    ...
    if ($music->config->ftp_upload == "on" && $music->config->ftp_username !== "" && $music->config->ftp_password !== "") {
        ...
        $delete = $s3->deleteObject($music->config->space_name, $filename);
        if ($delete == true) {
            return true;
        }
        return false;
        ...
        $delete = $s3->deleteObject($music->config->s3_bucket_name, $filename);
        if ($delete == true) {
            return true;
        }
        return false;
