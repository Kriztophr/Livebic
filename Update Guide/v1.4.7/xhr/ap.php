...
if ($option == "testS3") {
    include_once "assets/libs/S3.php";
    try {

        $s3 = new S3($music->config->amazone_s3_key, $music->config->amazone_s3_s_key);
        $buckets = $s3->listBuckets();
        if (!empty($buckets) && in_array($music->config->s3_bucket_name, $buckets)) {
            $data["status"] = 200;
            $array = [
...
if ($option == "test_spaces") {
    ...
    try {
        ...
        $s3 = new S3($music->config->spaces_key, $music->config->spaces_secret);

        $s3->setEndpoint($music->config->space_region.".digitaloceanspaces.com");
        $data["status"] = 200;
        $array = [
            "upload/photos/d-cover.jpg",
            "upload/photos/d-avatar.jpg",
...
if ($option == "test_ftp") {
    ...
    try {
        ...
        $array = ["upload/photos/d-cover.jpg", "upload/photos/d-avatar.jpg"];
        foreach ($array as $key => $value) {
            $upload = PT_UploadToS3($value, [
                "delete" => "no",
            ]);
