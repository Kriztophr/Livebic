...
if ($option == 'get_aamarpay') {
    if (!empty($_POST['amount']) && is_numeric($_POST['amount']) && $_POST['amount'] > 0 && !empty($_POST['name']) && !empty($_POST['email']) && !empty($_POST['phone'])) {
        ...
        curl_close($ch);
        $db->where('id',$music->user->id)->update(T_USERS,array('aamarpay_tran_id' => $tran_id));
        if ($music->config->aamarpay_mode == 'sandbox') {
...
if ($option == 'success_aamarpay') {
    if (!empty($_POST['amount']) && !empty($_POST['mer_txnid']) && !empty($_POST['pay_status']) && $_POST['pay_status'] == 'Successful') {
        $user = $db->objectBuilder()->where('aamarpay_tran_id',secure($_POST['mer_txnid']))->getOne(T_USERS);
        if (!empty($user)) {
            $amount   = (int)secure($_POST['amount']);
            $updateUser = $db
                    ->where("id", $user->id)
...
if ($option == 'cancel_aamarpay') {
    $db->where('id',$music->user->id)->update(T_USERS,array('aamarpay_tran_id' => ''));
    $data     = array(
        'status'   => 200,
        'message' => 'success'
    );
...
if ($option == 'success_ngenius') {
    if (!empty($_POST['ref'])) {
        $user = $db->objectBuilder()->where('ngenius_ref',secure($_POST['ref']))->getOne(T_USERS);
        if (!empty($user)) {
            $token = GetNgeniusToken();
            if (!empty($token->message)) {
                $data = array(
...
if ($option == 'get-profile') {
    if (empty($_POST['user_id']) || empty($_POST['fetch'])) {
        ...
        if (empty($errors)) {
            ...
            if(in_array('events',$fetch) && $music->config->event_system == 1){
                $all_events = array();
