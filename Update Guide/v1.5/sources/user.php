...
if (!CanUseThisFeature($userData->id,'who_can_station_import') && $music->config->radio_station_import == 'on') {
	$music->config->radio_station_import = 'off';
}
if (!CanUseThisFeature($userData->id,'who_can_event_system') && $music->config->event_system == '1') {
	$music->config->event_system = 0;
}
...
$userFinalData = [
	...
	'COUNT_FOLLOWERS' => number_format($db->where('following_id', $userData->id)->where("follower_id NOT IN (SELECT blocked_id FROM blocks WHERE user_id = $userData->id)")->getValue(T_FOLLOWERS, 'COUNT(*)')),
	'COUNT_FOLLOWING' => number_format($db->where('follower_id', $userData->id)->where("following_id NOT IN (SELECT blocked_id FROM blocks WHERE user_id = $userData->id)")->getValue(T_FOLLOWERS, 'COUNT(*)')),
	'COUNT_TRACKS' => number_format($db->where('user_id', $userData->id)->getValue(T_SONGS, 'COUNT(*)')),
