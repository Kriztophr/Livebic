...
if ($option == "upload_media") {
    if (
        ...
        if ($id !== $music->user->id) {
            $chat_exits = $db
                ->where("user_one", $music->user->id)
                ->where("user_two", $id)
                ->getValue(T_CHATS, "count(*)");
            if (!empty($chat_exits)) {
                $db->where("user_two", $music->user->id)
                    ->where("user_one", $id)
                    ->update(T_CHATS, ["time" => time()]);
                $db->where("user_one", $music->user->id)
                    ->where("user_two", $id)
                    ->update(T_CHATS, ["time" => time()]);
                if (
                    $db
                        ->where("user_two", $music->user->id)
                        ->where("user_one", $id)
                        ->getValue(T_CHATS, "count(*)") == 0
                ) {
                    $db->insert(T_CHATS, [
                        "user_two" => $music->user->id,
                        "user_one" => $id,
                        "time" => time(),
                    ]);
                }
            ...
            if (!empty($_FILES["media"]["tmp_name"])) {
                ...
                if (!empty($file_upload["filename"])) {
                    ...
                    $msg_exits = $db
                        ->where("from_id", $music->user->id)
                        ->where("to_id", $id)
                        ->where("text", $new_message)
        ...
        if ($id != $music->user->id) {
            ...
            if ($insert) {
                ...
                $data = [
                    ...
                    "message" => LoadPage("messages/ajax/outgoing", [
                        "ID" => $music->message->id,
                        "TEXT" => $music->message->text,
                    ]),
                ];
...
if ($option == "fetch") {
    ...
    $messages_html = GetMessages($_POST["id"], [
        "last_id" => $_POST["last_id"],
        "first_id" => $_POST["first_id"],
        "return_method" => "html",
    ]);
    if (!empty($messages_html)) {
        $html = LoadPage("messages/ajax/messages", [
    ...
    $users_html = GetMessagesUserList(["return_method" => "html", 'chat_id' => $_POST["id"]]);
...
if ($option == "search") {
    ...
    if (isset($_POST["keyword"])) {
        $users_html = GetMessagesUserList([
            "return_method" => "html",
            "keyword" => $_POST["keyword"],
        ]);
    }
    $data = ["status" => 200, "users" => $users_html];
}
...
if ($option == "delete_chat") {
    if (!empty($_POST["id"])) {
        ...
        $messages = $db
            ->where(
                "(from_id = {$music->user->id} AND to_id = {$id}) OR (from_id = {$id} AND to_id = {$music->user->id})"
            )
            ->get(T_MESSAGES);
        $update1 = [];
        $update2 = [];
        ...
        if (!empty($erase)) {
            $erase = implode(",", $erase);
            $final_query =
                "DELETE FROM " . T_MESSAGES . " WHERE id IN ($erase)";
        ...
        if (!empty($update1)) {
            ...
            $final_query =
                "UPDATE " .
                T_MESSAGES .
                " set `from_deleted` = '1' WHERE `id` IN({$update1}) ";
            $db->rawQuery($final_query);
        }
        if (!empty($update2)) {
            ...
            $final_query =
                "UPDATE " .
                T_MESSAGES .
                " set `to_deleted` = '1' WHERE `id` IN({$update2}) ";
            $db->rawQuery($final_query);
        }
        $delete_chats = $db->rawQuery(
            "DELETE FROM " .
                T_CHATS .
                " WHERE user_one = {$music->user->id} AND user_two = $id"
        );
    }
}
if ($option == "load_user_chat") {
    $data = ["status" => 400];
    if (!empty($_POST['time']) && is_numeric($_POST['time'])) {
        $users_html = GetMessagesUserList(array('return_method' => 'html'),20,Secure($_POST['time']));
        if (!empty($users_html)) {
...
if ($option == 'mark_read') {
    $data = ["status" => 200];
    $db->where("to_id",$music->user->id)->update(T_MESSAGES, array('seen' => time()));
}
?>