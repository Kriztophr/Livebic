...
if (in_array(true, $request)) {
   $errors[] = lang("Something went wrong Please try again later!");
	...
	$shell     = shell_exec("$ffmpeg_b -i $audio_file_full_path -map 0:a:0 -b:a ".$convert_speed."k $audio_output_mp3 2>&1");
    ...
    if (!empty($_POST['type']) && $_POST['type'] == 'story' && is_numeric($_POST['id'])) {
    	$db->where('id', Secure($_POST['id']))->where('user_id', $music->user->id);
		$update_data = array(
		    'audio' => $originalURL,
		);
