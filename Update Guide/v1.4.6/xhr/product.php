...
if ($option == "edit") {
    if (
        ...
        if (empty($data["message"])) {
            $db->where("id", $product->id)->update(T_PRODUCTS, [
                "title" => secure($_POST["title"]),
                "desc" => secure($_POST["desc"]),
                "price" => secure($_POST["price"]),
                "units" => secure($_POST["units"]),
...
if ($option == "download") {
    if (!empty($_POST["id"])) {
        ...
        if (!empty($music->purchase)) {
            $music->orders = $db
                ->where("hash_id", $music->purchase->order_hash_id)
                ->get(T_ORDERS);
            if (!empty($music->orders)) {
                ...
                foreach ($music->orders as $key => $music->order) {
                    ...
                    $music->html .=
                        ...
                        "</h6></td><td>" .
                        number_format(
                            $music->order->price / $music->order->units,
                            2,
                            "."
                        ) .
                        "</td><td>" .
                        ...
                        number_format($music->order->price, 2, ".") .
                        "</span></td></tr>";
                }
                ...
                $music->total_commission = number_format(
                    $music->total_commission,
                    2,
                    "."
                );
                $music->total_final_price = number_format(
                    $music->total_final_price,
                    2,
                    "."
                );
